...
@section('content')
<div class="space-y-6" x-data="{ deleteOpen: false, deleteUrl: '', deleteName: '' }">
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center justify-end gap-3">
                            ...
                            {{-- Deploy (git projects only) --}}
                            @if ($project->source_type === 'git')
                                <button type="button"
                                        @click="window.dispatchEvent(new CustomEvent('open-deploy', {detail: { id: {{ $project->id }}, name: @js($project->name), branch: @js($project->branch ?? 'main'), isGit: true, redeploy: true, startUrl: @js(route('projects.deploy', $project)), commandRunUrl: @js(route('projects.commands.run', $project)) }}))"
                            ...
                            {{-- Delete --}}
                            <button type="button"
                                    @click="deleteUrl = '{{ route('projects.destroy', $project) }}'; deleteName = '{{ addslashes($project->name) }}'; deleteOpen = true"
