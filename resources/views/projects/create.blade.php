...
@section('title', 'New Project — LaraHostPanel')
@section('heading', 'New Project')

@section('content')
<div class="mx-auto max-w-2xl">
    <form method="POST" action="{{ route('projects.store') }}" x-data="{ sourceType: 'local' }">
        @csrf
        ...
        <div class="space-y-6 rounded-xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
                ...
                <div class="mt-2 flex gap-4">
                    <label class="flex cursor-pointer items-center gap-2">
                        <input type="radio" name="source_type" value="local"
                               ...
                               {{ old('source_type', 'local') === 'local' ? 'checked' : '' }}
            ...
            <div x-show="sourceType === 'local'" x-transition>
                ...
                <input type="text" id="local_path" name="local_path" value="{{ old('local_path') }}"
            ...
            <div x-show="sourceType === 'git'" x-transition class="space-y-4">
                <div>
                    ...
                    <input type="text" id="git_url" name="git_url" value="{{ old('git_url') }}"
                           class="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 font-mono text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                    ...
                    <div>
                        ...
                        <select id="git_credential_id" name="git_credential_id"
                            ...
                            @foreach ($credentials as $cred)
                                <option value="{{ $cred->id }}" {{ old('git_credential_id') == $cred->id ? 'selected' : '' }}>
                                    {{ $cred->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" id="ip_address" name="ip_address" value="{{ old('ip_address', '0.0.0.0') }}" required
                           class="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 font-mono text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                </div>
                <div>
                    <label for="port" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Port</label>
                    <input type="number" id="port" name="port" value="{{ old('port') }}" required min="1" max="65535"
                           class="mt-1 block w-full rounded-lg border border-gray-300 bg-white px-3 py-2 font-mono text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-800 dark:text-white"
                           placeholder="8080">
                </div>
            </div>
            ...
            {{-- Auto deploy --}}
            <div class="space-y-3" x-data="{ autoDeploy: {{ old('auto_deploy') ? 'true' : 'false' }} }">
                ...
                <div x-show="autoDeploy" x-transition>
                    ...
                    <input type="number" id="auto_deploy_interval" name="auto_deploy_interval"
                           value="{{ old('auto_deploy_interval', 5) }}" min="1" max="1440"
            ...
            {{-- Auto start --}}
            <div class="space-y-3" x-data="{ autoStart: {{ old('auto_start') ? 'true' : 'false' }} }">
                <label class="flex cursor-pointer items-center gap-3">
